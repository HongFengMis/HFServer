...
io.on('connection', function(socket){
    ...
    socket.on("Client_Conf" , function(){
        var json = {};
        for(var i in IO.client){
            if(IO.client[i]) json[i] = 1;
        }
        io.to(IO.nm["guide"]).emit("Client_Conf", json);
    });
...
IO.Conn_client = function(ids, socket, callback){
    ...
    io.to(IO.nm["guide"]).emit("Client_join", {nm:ids.client});
    //通报
    ...
    socket.on('disconnect', function () {
        ...
        io.to(IO.nm["guide"]).emit("Client_exit", {nm:ids.client});
        IO.ClearRoom(ids.name);
        //通报
...
IO.Conn_guide = function(ids, socket, callback){
    ...
    socket.on('disconnect', function () {
        //序厅
        io.emit("select",{type:1});
        io.emit("select",{font:"00"});
        ...
        io.to(IO.nm["guide"]).emit("Guide_exit");
